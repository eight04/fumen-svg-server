...
function createInnerField(field) {
    ...
    for (var y = -1; y < FieldConstants$2.Height; y += 1) {
        for (var x = 0; x < FieldConstants$2.Width; x += 1) {
            var at = field.at(x, y);
            innerField.setNumberAt(x, y, _module_exports_$9.parsePiece(at));
        }
    }
    return innerField;
}
...
var PlayField = /** @class */ (function () {
    ...
    PlayField.loadInner = function (blocks, length) {
        ...
        for (var index = 0; index < len; index += 1) {
            ...
            field.set(index % 10, Math.floor((len - index - 1) / 10), _module_exports_$9.parsePiece(block));
        }
        return field;
    };
...
_module_exports_$6.createActionDecoder = function (width, fieldTop, garbageLine) {
    ...
    return {
        decode: function (v) {
            ...
            return {
                ...
                comment: isComment,
                lock: isLock,
                piece: __assign$2(__assign$2({}, coordinate), { type: type,
                    rotation: rotation }),
            };
        },
    };
...
_module_exports_$6.createActionEncoder = function (width, fieldTop, garbageLine) {
    var fieldMaxHeight = fieldTop + garbageLine;
    var numFieldBlocks = fieldMaxHeight * width;
    function encodePosition(operation) {
        ...
        var x = operation.x;
        var y = operation.y;
        if (!_module_exports_$9.isMinoPiece(type)) {
            x = 0;
            y = 22;
        }
...
_module_exports_$5.createCommentParser = function () {
    return {
        decode: function (v) {
            var str = '';
            var value = v;
...
var Quiz = /** @class */ (function () {
    ...
    Quiz.prototype.getOperation = function (used) {
        var usedName = _module_exports_$9.parsePieceName(used);
    ...
    Quiz.prototype.getHoldPiece = function () {
        ...
        return _module_exports_$9.parsePiece(name);
    };
...
var Field = /** @class */ (function () {
    ...
    Field.prototype.fill = function (operation, force) {
        ...
        this.field.fillAll(mino.positions(), _module_exports_$9.parsePiece(mino.type));
        return mino;
    };
    ...
    Field.prototype.at = function (x, y) {
        return _module_exports_$9.parsePieceName(this.field.getNumberAt(x, y));
    };
    Field.prototype.set = function (x, y, type) {
        this.field.setNumberAt(x, y, _module_exports_$9.parsePiece(type));
    };
...
var Mino = /** @class */ (function () {
    ...
    Mino.prototype.positions = function () {
        return _module_exports_$8.getBlockXYs(_module_exports_$9.parsePiece(this.type), _module_exports_$9.parseRotation(this.rotation), this.x, this.y).sort(function (a, b) {
            if (a.y === b.y) {
                return a.x - b.x;
            }
            return a.y - b.y;
    ...
    Mino.prototype.isValid = function () {
        try {
            _module_exports_$9.parsePiece(this.type);
            _module_exports_$9.parseRotation(this.rotation);
        }
        catch (e) {
            return false;
        }
...
var Page = /** @class */ (function () {
    ...
    Object.defineProperty(Page.prototype, "field", {
        ...
        set: function (field) {
            this._field = _module_exports_$8.createInnerField(field);
        },
        enumerable: true,
        configurable: true
    });
...
function innerDecode(data, fieldTop) {
    ...
    var pageIndex = 0;
    var prevField = _module_exports_$8.createNewInnerField();
    var store = {
        repeatCount: -1,
        refIndex: {
            comment: 0,
    ...
    var pages = [];
    var actionDecoder = _module_exports_$6.createActionDecoder(FieldConstants$1.Width, fieldTop, FieldConstants$1.GarbageLine);
    var commentDecoder = _module_exports_$5.createCommentParser();
    while (!buffer.isEmpty()) {
        ...
        if (store.quiz !== undefined) {
            ...
            if (store.quiz.canOperate() && action.lock) {
                if (_module_exports_$9.isMinoPiece(action.piece.type)) {
                    try {
        ...
        pages.push(new Page(pageIndex, currentFieldObj.field, currentPiece !== undefined ? _module_exports_$3.Mino.from({
            type: _module_exports_$9.parsePieceName(currentPiece.type),
            rotation: _module_exports_$9.parseRotationName(currentPiece.rotation),
            x: currentPiece.x,
            y: currentPiece.y,
...
function encode(pages) {
    ...
    var prevField = _module_exports_$8.createNewInnerField();
    var actionEncoder = _module_exports_$6.createActionEncoder(FieldConstants.Width, 23, FieldConstants.GarbageLine);
    var commentParser = _module_exports_$5.createCommentParser();
    var prevComment = '';
    var prevQuiz = undefined;
    var innerEncode = function (index) {
        var currentPage = pages[index];
        var field = currentPage.field;
        var currentField = field !== undefined ? _module_exports_$8.createInnerField(field) : prevField.copy();
        // フィールドの更新
        ...
        var piece = currentPage.operation !== undefined ? {
            type: _module_exports_$9.parsePiece(currentPage.operation.type),
            rotation: _module_exports_$9.parseRotation(currentPage.operation.rotation),
        ...
        if (prevQuiz !== undefined && prevQuiz.canOperate() && currentPage.flags.lock) {
            if (_module_exports_$9.isMinoPiece(piece.type)) {
                try {
...
_module_exports_.Field = _module_exports_$3.Field;
_module_exports_.Mino = _module_exports_$3.Mino;
_module_exports_.decoder = {
    decode: function (data) {
        return _module_exports_$2.decode(data);
    },
};
_module_exports_.encoder = {
    encode: function (data) {
        return "v115@" + _module_exports_$1.encode(data);
    },
};
